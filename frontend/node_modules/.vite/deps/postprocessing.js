import { BasicDepthPacking, BufferAttribute, BufferGeometry, Camera, ClampToEdgeWrapping, Color, Data3DTexture, DataTexture, DepthStencilFormat, DepthTexture, EventDispatcher, FileLoader, FloatType, LinearFilter, LinearMipmapLinearFilter, LinearSRGBColorSpace, Loader, LoadingManager, LuminanceFormat, Material, Matrix4, Mesh, MeshDepthMaterial, MeshNormalMaterial, NearestFilter, PerspectiveCamera, RGBADepthPacking, RGBAFormat, RGFormat, RedFormat, RepeatWrapping, SRGBColorSpace, Scene, ShaderMaterial, Texture, Uniform, UnsignedByteType, UnsignedInt248Type, UnsignedIntType, Vector2, Vector3, Vector4, WebGLRenderTarget } from "./three.js";
...
var revision = Number("160".replace(/\D+/g, ""));
function updateFragmentShader(fragmentShader) {
	if (revision < 154) return fragmentShader.replace("colorspace_fragment", "encodings_fragment");
...
var EffectComposer = class {
	...
	*/
	createDepthTexture() {
		...
		} else depthTexture.type = UnsignedIntType;
		return depthTexture;
	}
	/**
	...
	*/
	addPass(pass, index) {
		...
		if (pass.needsDepthTexture || this.depthTexture !== null) if (this.depthTexture === null) {
			...
			for (pass of passes) pass.setDepthTexture(depthTexture);
		} else pass.setDepthTexture(this.depthTexture);
	}
	/**
	* Removes a pass.
	*
		...
		if (index !== -1 && passes.splice(index, 1).length > 0) {
			if (this.depthTexture !== null) {
				...
				if (!passes.reduce(reducer, false)) {
					if (pass.getDepthTexture() === this.depthTexture) pass.setDepthTexture(null);
	...
	*/
	render(deltaTime) {
		...
		for (const pass of this.passes) if (pass.enabled) {
...
var blendFunctions = /* @__PURE__ */ new Map([
	[BlendFunction.ADD, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,x+y,opacity);}"],
	[BlendFunction.ALPHA, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,y,min(y.a,opacity));}"],
	[BlendFunction.AVERAGE, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,(x+y)*0.5,opacity);}"],
	[BlendFunction.COLOR, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec3 xHSL=RGBToHSL(x.rgb);vec3 yHSL=RGBToHSL(y.rgb);vec3 z=HSLToRGB(vec3(yHSL.rg,xHSL.b));return vec4(mix(x.rgb,z,opacity),y.a);}"],
	[BlendFunction.COLOR_BURN, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec4 z=mix(step(0.0,y)*(1.0-min(vec4(1.0),(1.0-x)/y)),vec4(1.0),step(1.0,x));return mix(x,z,opacity);}"],
	[BlendFunction.COLOR_DODGE, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec4 z=step(0.0,x)*mix(min(vec4(1.0),x/max(1.0-y,1e-9)),vec4(1.0),step(1.0,y));return mix(x,z,opacity);}"],
	[BlendFunction.DARKEN, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,min(x,y),opacity);}"],
	[BlendFunction.DIFFERENCE, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,abs(x-y),opacity);}"],
	[BlendFunction.DIVIDE, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,x/max(y,1e-12),opacity);}"],
	[BlendFunction.DST, null],
	[BlendFunction.EXCLUSION, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,(x+y-2.0*x*y),opacity);}"],
	[BlendFunction.HARD_LIGHT, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec4 a=min(x,1.0),b=min(y,1.0);vec4 z=mix(2.0*a*b,1.0-2.0*(1.0-a)*(1.0-b),step(0.5,y));return mix(x,z,opacity);}"],
	[BlendFunction.HARD_MIX, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,step(1.0,x+y),opacity);}"],
	[BlendFunction.HUE, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec3 xHSL=RGBToHSL(x.rgb);vec3 yHSL=RGBToHSL(y.rgb);vec3 z=HSLToRGB(vec3(yHSL.r,xHSL.gb));return vec4(mix(x.rgb,z,opacity),y.a);}"],
	[BlendFunction.INVERT, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,1.0-y,opacity);}"],
	[BlendFunction.INVERT_RGB, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,y*(1.0-x),opacity);}"],
	[BlendFunction.LIGHTEN, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,max(x,y),opacity);}"],
	[BlendFunction.LINEAR_BURN, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,clamp(y+x-1.0,0.0,1.0),opacity);}"],
	[BlendFunction.LINEAR_DODGE, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,min(x+y,1.0),opacity);}"],
	[BlendFunction.LINEAR_LIGHT, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,clamp(2.0*y+x-1.0,0.0,1.0),opacity);}"],
	[BlendFunction.LUMINOSITY, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec3 xHSL=RGBToHSL(x.rgb);vec3 yHSL=RGBToHSL(y.rgb);vec3 z=HSLToRGB(vec3(xHSL.rg,yHSL.b));return vec4(mix(x.rgb,z,opacity),y.a);}"],
	[BlendFunction.MULTIPLY, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,x*y,opacity);}"],
	[BlendFunction.NEGATION, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,1.0-abs(1.0-x-y),opacity);}"],
	[BlendFunction.NORMAL, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,y,opacity);}"],
	[BlendFunction.OVERLAY, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec4 z=mix(2.0*y*x,1.0-2.0*(1.0-y)*(1.0-x),step(0.5,x));return mix(x,z,opacity);}"],
	[BlendFunction.PIN_LIGHT, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec4 y2=2.0*y;vec4 z=mix(mix(y2,x,step(0.5*x,y)),max(vec4(0.0),y2-1.0),step(x,(y2-1.0)));return mix(x,z,opacity);}"],
	[BlendFunction.REFLECT, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec4 z=mix(min(x*x/max(1.0-y,1e-12),1.0),y,step(1.0,y));return mix(x,z,opacity);}"],
	[BlendFunction.SATURATION, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec3 xHSL=RGBToHSL(x.rgb);vec3 yHSL=RGBToHSL(y.rgb);vec3 z=HSLToRGB(vec3(xHSL.r,yHSL.g,xHSL.b));return vec4(mix(x.rgb,z,opacity),y.a);}"],
	[BlendFunction.SCREEN, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,x+y-min(x*y,1.0),opacity);}"],
	[BlendFunction.SOFT_LIGHT, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec4 y2=2.0*y;vec4 w=step(0.5,y);vec4 z=mix(x-(1.0-y2)*x*(1.0-x),mix(x+(y2-1.0)*(sqrt(x)-x),x+(y2-1.0)*x*((16.0*x-12.0)*x+3.0),w*(1.0-step(0.25,x))),w);return mix(x,z,opacity);}"],
	[BlendFunction.SRC, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return y;}"],
	[BlendFunction.SUBTRACT, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){return mix(x,max(x+y-1.0,0.0),opacity);}"],
	[BlendFunction.VIVID_LIGHT, "vec4 blend(const in vec4 x,const in vec4 y,const in float opacity){vec4 z=mix(max(1.0-min((1.0-x)/(2.0*y),1.0),0.0),min(x/(2.0*(1.0-y)),1.0),step(0.5,y));return mix(x,z,opacity);}"]
]);
	...
	*/
	constructor(colorOutput = false, luminanceRange = null) {
		super({
			...
			defines: { THREE_REVISION: "160".replace(/\D+/g, "") },
			uniforms: {
				inputBuffer: new Uniform(null),
...
var BloomEffect = class extends Effect {
	...
	*/
	constructor({ blendFunction = BlendFunction.SCREEN, luminanceThreshold = .9, luminanceSmoothing = .025, mipmapBlur = false, intensity = 1, radius = .85, levels = 8, kernelSize = KernelSize.LARGE, resolutionScale = .5, width = Resolution.AUTO_SIZE, height = Resolution.AUTO_SIZE, resolutionX = width, resolutionY = height } = {}) {
...
var DepthOfFieldEffect = class extends Effect {
	...
	* @param {Number} [options.height=Resolution.AUTO_SIZE] - Deprecated. Use resolutionY instead.
	*/
	constructor(camera, { blendFunction, worldFocusDistance, worldFocusRange, focusDistance = 0, focalLength = .1, focusRange = focalLength, bokehScale = 1, resolutionScale = 1, width = Resolution.AUTO_SIZE, height = Resolution.AUTO_SIZE, resolutionX = width, resolutionY = height } = {}) {
		...
		if (worldFocusDistance !== void 0) cocMaterial.worldFocusDistance = worldFocusDistance;
		if (worldFocusRange !== void 0) cocMaterial.worldFocusRange = worldFocusRange;
	...
	*/
	calculateFocusDistance(target) {
		const camera = this.camera;
		return viewZToOrthographicDepth(-camera.position.distanceTo(target), camera.near, camera.far);
	}
	/**
	* Sets the depth texture.
	*
...
var GodRaysEffect = class extends Effect {
	...
	*/
	update(renderer, inputBuffer, deltaTime) {
		...
		this.screenPosition.set(Math.min(Math.max((v.x + 1) * .5, -1), 2), Math.min(Math.max((v.y + 1) * .5, -1), 2));
		if (this.blurPass.enabled) this.blurPass.render(renderer, renderTargetA, renderTargetA);
	...
	*/
	configureTetrahedralInterpolation() {
		...
		if (lut !== null) {
			...
			if (lut.source === void 0) lut.needsUpdate = true;
		}
	}
	/**
	...
	update(renderer, inputBuffer, delta) {
		...
		if (this.active) {
			...
			if (uActive.value) {
				...
				this.screenPosition.set((v2.x + 1) * .5, (v2.y + 1) * .5);
			}
	...
	*/
	constructor(texelSize = new Vector2(), mode = EdgeDetectionMode.COLOR) {
		super({
			...
			defines: {
				THREE_REVISION: "160".replace(/\D+/g, ""),
		...
		return new Promise((resolve, reject) => {
			internalManager.onError = (url2) => {
				...
				if (onError !== null) {
					...
					resolve();
				} else reject(`Failed to load ${url2}`);
			};
...
`;
		...
		if (mainImageExists) {
			if (effect.inputColorSpace !== null && effect.inputColorSpace !== data.colorSpace) fragmentMainImage += effect.inputColorSpace === "srgb" ? "color0 = LinearTosRGB(color0);\n	" : "color0 = sRGBToLinear(color0);\n	";
